...
def inverse_qft(psi: np.ndarray) -> np.ndarray:
    ...
    for i in range(n_qubits//2 -1, -1, -1):
        psi = do_the_swap(psi, i, n_qubits - i - 1)
    print("time swap", time() - start_time)
    ...
    print("time bigloop", time() - start_time)
...
def qft_old(n_qubits: int) -> np.ndarray:
...
def inverse_qft_old(n_qubits: int) -> np.ndarray:
    H = hadamard()
...
if __name__ == "__main__":
    ...
    print(np.linalg.norm(ket0_F_inv - qft_linalg.conj().T @ ket0)) # TODO: this turns out nonzero??  what is going on??
